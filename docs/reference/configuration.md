...
The generated `.env.testing` contract forces `APP_MAINTENANCE_ENABLED=false`, `APP_MAINTENANCE_DRIVER=memory`, and `APP_MAINTENANCE_STORE=default`, including named-App forms. Shared deployment state therefore cannot unexpectedly gate HTTP requests in tests.
