...
Maintenance mode has a live command path instead of requiring an environment edit. Run `./bin/app maintenance:enable` to pause new web traffic and `./bin/app maintenance:disable` to resume it. The commands call the running App through an authenticated control endpoint. Requests already inside an application handler may finish; queue workers and scheduled tasks continue normally. Browser requests receive the starter-kit-independent maintenance page, while API requests receive JSON. The default memory backend is intended for local development; production deployments should use a distributed named cache store. See [Deploy an App](/operations/deployment-basics#use-maintenance-mode) for the operational workflow, configuration, and route exceptions.
