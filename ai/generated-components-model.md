...
Schedule nuance: scheduler runtime code lives in `internal/schedules`, while generated schedule implementations may be colocated with their owning domain package. `forj make:schedule reports:daily --every 24h` should create a domain-owned schedule such as `internal/reports/daily_schedule.go`, register it in `app/schedules.go`, and wire its provider through `app/wire/inject_schedules_app.go`. For an additional app, use the owning app's `app/<app>/schedules.go` and `app/<app>/wire/inject_schedules_app.go`. Do not document `internal/scheduler`; that package name was replaced to avoid conflicting with the `github.com/goforj/scheduler/v2` library import.
